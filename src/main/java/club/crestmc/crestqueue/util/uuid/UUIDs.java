package club.crestmc.crestqueue.util.uuid;
...
public class UUIDs {
    ...
    public static void getNameFromUUID(UUID uuid, Consumer<String> consumer) {
        if (!cachedPlayers.containsValue(uuid)) {
            ...
            consumer.accept(cachedPlayers.keySet().stream().filter(username -> cachedPlayers.get(username).equals(uuid)).findFirst().orElse(null));
        }
    }
